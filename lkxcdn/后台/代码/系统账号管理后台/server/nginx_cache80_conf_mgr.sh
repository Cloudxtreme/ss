...
add)
	...
	check=`grep $clientdomain $confbe|wc -l`;
	echo $check

	if [ $check -eq 0 ]
	then
		echo "#$clientname""_""$clientdomain""_begin80" >> $confbe
		echo "server {" >> $confbe
		echo "	listen 80;" >> $confbe
		echo "	server_name *."$clientdomain";" >> $confbe
		echo "	location / {" >> $confbe
		echo "		proxy_cache cache_one;" >> $confbe
		echo "		proxy_cache_valid 200 3650d;" >> $confbe
		echo '		proxy_cache_key $host$uri;' >> $confbe
		echo '		proxy_set_header Host $host;' >> $confbe
		echo '		proxy_set_header X-Forwarded-For $remote_addr;' >> $confbe
		echo "		proxy_pass http://cdn_hotfile_fs_backend_server/"$clientname'/$host/$uri;' >> $confbe
		...
		echo "	} " >> $confbe
		echo "}" >> $confbe
		echo "#$clientname""_""$clientdomain""_end80" >> $confbe
	fi
...
del)
	...
	n1=`grep -n "#$clientname""_""$clientdomain""_begin80" $confbe|awk -F ':' '{print $1}'`
	n2=`grep -n "#$clientname""_""$clientdomain""_end80" $confbe|awk -F ':' '{print $1}'`
