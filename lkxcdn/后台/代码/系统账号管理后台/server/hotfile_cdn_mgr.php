...
function add_nginx()
{
	...
	$cmd = "/var/www/html/webadmin/server/nginx_cache_conf_mgr.sh add $client_name $client_port";
...
function del_nginx()
{
	...
	$cmd = "/var/www/html/webadmin/server/nginx_cache_conf_mgr.sh del $client_name";
	this_server_run_cmd($cmd);	
}
...
function check_nginx()
{
        ...
        $cmd = "/opt/nginx_cache/nginx/sbin/nginx -t -c /opt/nginx_cache/nginx/conf/nginx.conf > /tmp/xxx 2>&1; ";
        $cmd .= "grep \#$client_name /opt/nginx_cache/nginx/conf/nginx.conf >> /tmp/xxx; cat /tmp/xxx";
	file_node_server_run_cmd('电信', $cmd);
	file_node_server_run_cmd('网通', $cmd);
	file_node_server_run_cmd('BGP', $cmd);			
	file_node_server_run_cmd('移动', $cmd);
...
{/*
	...
	file_node_server_run_cmd('移动', $cmd);
	file_node_server_run_cmd('长城宽带', $cmd);						
	file_node_server_run_cmd('教育', $cmd);						*/
...
{/*
	...
	$cmd = "/var/www/html/webadmin/server/nginx_cache_tools_conf_mgr.sh add $client_name";
	this_server_run_cmd($cmd);	*/
...
{/*
	...
	$cmd = "/var/www/html/webadmin/server/nginx_cache_tools_conf_mgr.sh del $client_name";
	this_server_run_cmd($cmd);	*/
...
function add_client()
{
	...
	$query = "select * from `user` where `user` = '$client_name' or `nginxport` = $client_port";
	if( ! ($result = $dbobj->query($query)) ) 
	{
		print($dbobj->error()."\n");
		exit;
	...
	if( ! mysql_num_rows($result) )
	{
		...
		$query = "insert into `user`(`user`, `pass`, `type`, `nginxport`, `status`, `path`, `stats`, `desc`) 
							values('$client_name', md5('$ftp_pass'), 'cdnfile_hot', '$client_port', 'true', '$path', '$stats', '$client_desc');";
		print("$query\n");
	...
	{
		print("user $client_name or nginxport $client_port already exists\n");
	}	

	//$query = "select * from server_list where `type` = 'node' and `nettype` != '移动' and lower(`nettype`) != 'BGP'";
