...
print("$userip $user_nettype $user_zone <br><br>");
...
$query = "SELECT $global_databasename.server_list.ip, $global_databasename.user_nginx.port, nettype, match1, match2, 
		$global_databasename.server_list.desc
...
while( ($row = mysql_fetch_array($result)) ) 
{
	...
	$match2 = $row['match2'];
	$desc = $row['desc'];
	$cdn_node[$nettype][] = array( 'ip' => $ip, 'port' => $port, 'match1' => $match1, 'match2' => $match2, 'desc' => $desc );
}
