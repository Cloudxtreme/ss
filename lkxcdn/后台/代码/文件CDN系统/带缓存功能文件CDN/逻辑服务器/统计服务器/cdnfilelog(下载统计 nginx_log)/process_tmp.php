<?php
	...
	function parse_each_record($dbobj, $args)
	{
		...
		while(!feof($handle))
		{
			...
			$log_date = $file_status[0];
			$arr_date = explode(":", $log_date);
			$log_hour = $arr_date[1];
			$log_min = $arr_date[2];
			$log_min = floor($log_min / 5) * 5;
			$log_time = "$log_hour:$log_min:00";
			...
			if(!isset($sql_data["$hostname"]["$ip_nettype"]["$log_time"]))
			{
				$sql_data["$hostname"]["$ip_nettype"]["$log_time"]["bandwidth"] = 0;
				$sql_data["$hostname"]["$ip_nettype"]["$log_time"]["flow"] = 0;
			}
			$sql_data["$hostname"]["$ip_nettype"]["$log_time"]["bandwidth"] += $bandwidth;
			$sql_data["$hostname"]["$ip_nettype"]["$log_time"]["flow"] += $flow;
		...
		{
			foreach($data as $nettype=>$td)
			{
				foreach($td as $log_time=>$fb)
				{
					$bandwidth = round($fb["bandwidth"]);
					$flow = $fb["flow"];

					if(!strlen($val))
						$val = "('$parse_user','$hostname','$nettype',$bandwidth,$flow,'$log_time')"; 
					else
						$val .= "\n,('$parse_user','$hostname','$nettype',$bandwidth,$flow,'$log_time')";
				}
			}
		}
		if(strlen($val))
	...
	$parse_date = "2014-04-09";//$argv[1];
	$parse_hour = "";//$argv[2];
	$parse_group = "";//$argv[3];
	$parse_user = "gztizi";//$argv[4];
	$parse_file = "/opt/cdnfilelog/logs/2014-04-09/gztizi.log";//$argv[5];
