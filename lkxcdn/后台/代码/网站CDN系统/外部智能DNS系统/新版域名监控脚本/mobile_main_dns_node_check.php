...
//get zone table
$query = "select * from zone_table where `nettype` = '移动';";
...
foreach( $zone_table as $tablename ) {
	...
	foreach( $zone_table_info[$tablename] as $name => &$ipinfos ) {
		...
		$mobile_backup_ips = array();
		foreach( $nettype_iplist[$checktype] as $ip ) {
			if( ! array_key_exists($ip, $error_node_list) ) {
				$mobile_backup_ips[$ip] = $ip;
			}
		}
		if( count($mobile_backup_ips) > 0 ) {
			//随机取出一个移动节点
			$backup_ip = array_rand($mobile_backup_ips);
		}
