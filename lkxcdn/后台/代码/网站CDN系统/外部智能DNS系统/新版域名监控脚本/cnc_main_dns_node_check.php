...
//中文UTF-8
/*
这个脚本程序负责检查所有节点是否正常工作，如果出现故障
则对节点的地区表的status设置为false，如果整个节点出现故障，
则由其它节点覆盖，如果没有其它节点覆盖则暂由电信节点覆盖
*/
...
$error_node_list = array(); //记录有故障的ip列表
...
//get zone table
$query = "select * from zone_table where `nettype` = '网通';";
...
foreach( $zone_table as $tablename ) {
	...
	foreach( $zone_table_info[$tablename] as $name => &$ipinfos ) {
		...
		$cnc_backup_ips = array();
		foreach( $nettype_iplist[$checktype] as $ip ) {
			if( ! array_key_exists($ip, $error_node_list) ) {
				$cnc_backup_ips[$ip] = $ip;
			}
		}
		if( count($cnc_backup_ips) > 0 ) {
			//随机取出一个节点
			$backup_ip = array_rand($cnc_backup_ips);
		}
