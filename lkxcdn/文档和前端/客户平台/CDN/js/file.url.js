			$(function(){
				 check_login_user_acc( $("#_login_type").val(), 'file', 4, 45 );
			   ...
			   $.post("../function/file.url.fun.php",{ "user":user,"get_type":settype },function(data){
			   			var arr = data.split("***");
			   		  var zone = eval("("+arr[1]+")");
      	 ...
      	 var getOffDays =  mmSec / 3600000 / 24; 
			   ...
			   if( zone_str.length <= 2 || channel_str.length <= 2 ){ alert('请先选择完整的查询条件'); return; }
			    ...
			    $.post("../function/file.url.fun.php",{"user":user,"get_type":settype,"time":time_str,"channel":channel_str,"zone":zone_str,"url_type":url_type,"keyword":custom_key},function(data){
			    		   var arr = data.split("***");
			     _request_flow = eval("("+arr[0]+")");
			 	  _request_count = eval("("+arr[1]+")");
