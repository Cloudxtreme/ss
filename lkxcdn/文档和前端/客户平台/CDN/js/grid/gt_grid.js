...
//var cellF =  Sigma.U.getParentByTagName("td",null, event);
	...
	request : function(url,reqParam,parameterType , onSuccess,onFailure){
				...
				if (url) {
					try{
						...
						// queryId设置
						var p=grid.queryParameters;
						if(!p.queryId) p.queryId=grid.queryId;
						grid.ajax.setQueryParameters(p);
	...
	load : function(recount,force){
		...
		if (recount===true) {
			reqParam[this.CONST.pageInfo].totalRowNum=-1;
			reqParam[this.CONST.pageInfo].pageNum=1;
			reqParam[this.CONST.pageInfo].totalPageNum=0;
			reqParam[this.CONST.pageInfo].startRowNum=1;
			reqParam[this.CONST.pageInfo].endRowNum=0;
		}
...
//load, changePage
	...
	getFirstRow :function(){
			if(this.gridTbodyList.length==0) {
				return null;
			}
