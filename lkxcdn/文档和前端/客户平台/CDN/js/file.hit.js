			$(function(){
				 check_login_user_acc( $("#_login_type").val(), 'file', 4, 43 );
			   ...
			   $.post("../function/file.hit.fun.php",{ "user":user,"get_type":settype },function(data){
			   			var arr = data.split("***");
			   		  var zone = eval("("+arr[1]+")");
      	 ...
      	 var getOffDays =  mmSec / 3600000 / 24; 
			   if( getOffDays > 31 ){ alert('查询时间段不能超过31天'); return; } 
				 ...
				 var loading  = new ol.loading({id:"_container"});
				 loading.show();
			   ...
			   for( i = 0; i < channel.length; i++ ){     
			    ...
			    $.post("../function/file.hit.fun.php",{"user":user,"get_type":settype,"time":time_str,"channel":channel_str,"zone":zone_str},function(data){
					    		   var arr = data.split("***");
