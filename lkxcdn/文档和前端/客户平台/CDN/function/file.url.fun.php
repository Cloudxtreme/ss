...
foreach( $months as $month => $mdays ) 
{
	...
	{
	case "all":
		$query = gensql_hostname_urlday_cnt($month, $mdays, $channels, "like","%");
		break;
	case "custom":
		$key = array();
		...
		{
			case "in":
				$query = gensql_hostname_urlday_cnt($month, $mdays, $channels,"like","%$key[1]%");
				break;
			
			case "not_in":
				$query = gensql_hostname_urlday_cnt($month, $mdays, $channels,"not like","%$key[1]%");
				break;
...
if(count($days_urlreq_print) ==0)
{
	$days_urlreq_print[] = array('url' => '#', 'flow' => '-', 'cnt' => '-');
}
...
foreach( $months as $month => $mdays ) 
{
	...
	{
	case "all":
		$query = gensql_hostname_urlday_req($month, $mdays, $channels, "like","%");
		break;
	case "custom":
		$key = array();
		$key = json_decode($_POST['keyword']);
		...
		{
			case "in":
				$query = gensql_hostname_urlday_req($month, $mdays, $channels,"like","%$key[1]%");
				break;
			
			case "not_in":
				$query = gensql_hostname_urlday_req($month, $mdays, $channels,"not like","%$key[1]%");
				break;
...
if(count($days_urlcnt_print) ==0)
{
	$days_urlcnt_print[] = array('url' => '#', 'flow' => '-', 'cnt' => '-');
}
...
function gensql_hostname_urlday_req($table, $mdays, $channels, $action, $key)
{
	...
	$query .= "and `url` $action '$key'";
	$query .= " and `top_type` = 'topcnt' group by hostname,url order by `sum(cnt)` desc LIMIT 10 OFFSET 0;";
	return $query;
}

function gensql_hostname_urlday_cnt($table, $mdays, $channels, $action, $key)
{
	$query = "select hostname,url,sum(cnt),sum(send) from `$table".'_url`'." where `hostname` in (";
	foreach( $channels as $hostname ) {
		$query .= "'$hostname',";
