...
$query = "select * from `node_disk_data`;";
...
$db->query('TRUNCATE TABLE `node_disk_data`');

foreach( $days as $day ) {
	$query = "CREATE TABLE IF NOT EXISTS `node_disk_data_$day` (
			`id` int(11) NOT NULL AUTO_INCREMENT,
			`sid` char(50) NOT NULL,
			`dev` varchar(50) NOT NULL,
			`uper` smallint(20) NOT NULL,
...
foreach( $rows as $row ) {
	...
	$timestamp = "$day $time";
	$query = "insert `node_disk_data_$day` (`sid`, `dev`, `uper`, `timestamp`) 
			values('$sid', '$dev', '$uper', '$timestamp');";
	//print("$query\n");
	$db->query($query);
}
