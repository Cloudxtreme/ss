...
define("QUENE_NAME","pppoe_mgr_user_for");
define("QUENE_SERVER","mid.pppoe.rightgo.net");
define("QUENE_PORT","12001");
define("SLEEP_TIME",500000);

openlog("forbiddenuser",  LOG_ODELAY, LOG_LOCAL3);
prostart("forbiddenuser",getmypid());	
...
while(1)
{
	...
	else
	{
		...
		if(!$result)
		{
			$user->updateManage(User::COMMAND_FOR,false);
			continue;
		}
		//print_r($user);
		$result = $user->handleCommand(User::COMMAND_FOR);
		if($result)
		{
			$user->updateManage(User::COMMAND_FOR,true);
			syslog(LOG_INFO, "forbidden user:".$user->tostring()." succeed");
		}
		else
		{
			$user->updateManage(User::COMMAND_DEL,false);
			syslog(LOG_INFO, "forbidden user:".$user->tostring()." fail");
		}
	}
}
closelog();
