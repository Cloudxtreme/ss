...
define("QUENE_NAME","pppoe_mgr_user_res");
define("QUENE_SERVER","mid.pppoe.rightgo.net");
define("QUENE_PORT","12001");
define("SLEEP_TIME",500000);

openlog("resetuser",  LOG_ODELAY, LOG_LOCAL3);
prostart("resetuser",getmypid());
...
while(1)
{
	...
	else
	{
		...
		if(!$result)
		{
			$user->updateManage(User::COMMAND_RES,false);
			continue;
		}
		//print_r($user);
		$result = $user->handleCommand(User::COMMAND_RES);
		if($result)
		{
			$user->updateManage(User::COMMAND_RES,true);
			syslog(LOG_INFO, "reset user:".$user->tostring()." succeed");
		}
		else
		{
			$user->updateManage(User::COMMAND_RES,false);
			syslog(LOG_INFO, "reset user:".$user->tostring()." fail");
		}
	}
}
closelog();
