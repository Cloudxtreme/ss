...
class User
{
	...
	public function tostring()
	{
		...
		if($this->radius != null)
		{
			$radius_server = $this->radius->server;
		}
		return "[username:{$this->username}]--[password:{$this->password}]--[radius:{$radius_server}]--[mealid:{$this->mealid}]";
	}
	...
	{
		$head = $command;
		$content = "username[{$this->username}],password[{$this->password}],radius[{$this->radius->server}],startDate[{$this->startDate}],endDate[{$this->endDate}],mealid[{$this->mealid}]";
		mylog($head,$content);
		...
		switch($command)
		{
			...
			case self::COMMAND_SIM:
				...
				break;
			case self::COMMAND_RES:
				$statement = $this->resetUser();
				break;
	...
	public function updateManage($command,$result)
	{
		...
		switch($command)
		{
			...
			case self::COMMAND_SIM:
				...
				break;	
			case self::COMMAND_RES:
				$statement = $this->resetUser_result($status,$message);
				break;	
			default:
				$this->message =  "user command error\n";
				return false;
...
//		$meal->dataLimit($this->username);
	...
	private function resetUser()
	{
		...
		//$statement = "call deleteuser('{$this->username}')";
		$statement = "update radacct set acctstoptime=FROM_UNIXTIME(UNIX_TIMESTAMP(acctstarttime)+acctsessiontime),acctterminatecause='program' where acctstoptime is null and username='{$this->username}'";
		return $statement;
	}
	...
	private function resetUser_result($status,$message)
	{
		
                $head = "reset";
                mylog($head,"reset user[$this->username] {$status}");
		$statement = $this->user_result($status,$message); 
		return $statement;
	}
