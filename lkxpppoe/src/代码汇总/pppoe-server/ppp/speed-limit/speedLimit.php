...
openlog("speed-limit",  LOG_ODELAY, LOG_LOCAL3);
...
if($downloadspeed>500000 && $uploadspeed >250000)
{
	//if speed limit larger than 500M ,no limit
	return;
}
...
$uploadspeed = ceil($uploadspeed*$factor*0.4);
...
mysql_pconnect($center_db["server"],$center_db["username"],$center_db["password"]) or die("Could not connect: " . mysql_error());
mysql_select_db($center_db["manage_db"]);
$result = mysql_query("select ipaddr from  speedtest");
while($row = mysql_fetch_array($result,MYSQL_ASSOC))
{
	$speedtests[] = $row["ipaddr"];
}
mysql_free_result($result);
*/

limit($eth,$uploadspeed,$downloadspeed,$ipaddrs,$speedtests);
closelog();

function limit($eth,$upload,$download,$ipaddrs,$speedtests)
{
	$inter_download = $download*4;
	$speedtest_download = ceil($download*1.4);
//	echo $eth."\n";
//	echo $upload."\n";
//	echo $download."\n";
//	echo $inter_download."\n";
...
//	var_dump($ipaddrs);
	...
	system("/sbin/tc class add dev $eth parent 1: classid 1:1 htb rate {$download}kbit burst 1540 prio 2");
	system("/sbin/tc class add dev $eth parent 1: classid 1:2 htb rate {$inter_download}kbit burst 1540 prio 2");
	system("/sbin/tc class add dev $eth parent 1: classid 1:3 htb rate {$speedtest_download}kbit burst 1m prio 1");	
/*
	...
	foreach($speedtests as $ipaddr)
	{
		$commnad = "/sbin/tc filter add dev $eth  parent 1:0 protocol ip prio 1 u32 match ip src {$ipaddr} flowid 1:3";
		//syslog(LOG_INFO, "speed-test-command:{$commnad}");
		system($commnad);
	}
*/
//upload
