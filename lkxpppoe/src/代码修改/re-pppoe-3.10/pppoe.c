...
***********************************************************************/
void
session(PPPoEConnection *conn ,char * addr)
{
	//hezuoxiang declaration
	long uploadLimit = DEFAULT_SPEED_LIMIT_UPLOAD;
	long downloadLimit = DEFAULT_SPEED_LIMIT_DOWNLOAD;
	long packetLenth = 0;		//fine adjustment
	
	//get the speed limit value from shm
	char mac[20];
  sprintf(mac,"%02x:%02x:%02x:%02x:%02x:%02x",conn->peerEth[0], conn->peerEth[1], conn->peerEth[2],conn->peerEth[3], conn->peerEth[4], conn->peerEth[5]);
	
	speedLimit * object = addr_get_limit(mac);
	if(object != (speedLimit *)-1)
	{
		uploadLimit = (long)object->uploadSpeed;
		downloadLimit = (long)object->downloadSpeed;
		
		int pid = getpid();
		object->pid = pid;
		testlog("get speed limi succeed.\n");
	}
	else
	{
		testlog("get speed limi fail.\n");
	}
	
	//syslog(LOG_ERR, "client addriss:%s\n",addr);
	//syslog(LOG_ERR, "client ifName:%s\n",conn->ifName);
	
	long downloadByte = 0;
	long uploadByte = 0;
	
	long lastTime = getUtime();
	long nowTime = lastTime;
	
	long timeOfSec = 1000;
	long divide = 1;	//divide 1 second into 4 
	
	long uploadLimitDivide = (long)uploadLimit/divide;
	long downloadLimitDivide = (long)downloadLimit/divide;
	long timeDivide = (long)timeOfSec/divide;
	...
	/* Drop privileges */
	dropPrivs();
	//after dropPrives,user change to be nobody,and then no right to write file
	...
	{
		//char whilelog[100] = "hezuoxiang:while is in";
		//testlog(whilelog);
		if (conn->synchronous) 
		{
	...
	for (;;)
	{
		if(downloadByte>=downloadLimitDivide || uploadByte>=uploadLimitDivide)
		{
			nowTime = getUtime();
			long timePast = timeCompare(nowTime,lastTime);
			if(timePast >= timeDivide)
			{
				lastTime = nowTime;
				downloadByte = 0;
				uploadByte = 0;
				timePast = 0;
				//syslog(LOG_ERR, "hezuoxiang:stdio is in");
			}
			else
			{
				usleep((timeDivide-timePast)*1000);
			}
			continue;
		}
		...
		if (r == 0)
		{ /* Inactivity timeout */
			syslog(LOG_ERR, "Inactivity timeout... something wicked happened on session %d",(int) ntohs(conn->session));
			sendPADT(conn, "RP-PPPoE: Inactivity timeout");
			exit(EXIT_FAILURE);
		}

		/* Handle ready sockets */
		if(downloadByte<downloadLimitDivide)
		{
			if (FD_ISSET(0, &readable))
			{
				...
				if (conn->synchronous)
				{
					length = syncReadFromPPP(conn, &packet);
				}
				else
				{
					length = asyncReadFromPPP(conn, &packet);
				}
				downloadByte = downloadByte+length;
				//syslog(LOG_ERR, "downloadByte:%d",length);
				//syslog(LOG_ERR, "packet:%d",packet.length/8);
			}
		}
		
		if(uploadByte<uploadLimitDivide)
		{
			if (FD_ISSET(conn->sessionSocket, &readable)) 
			{
				...
				do 
				{
					...
					if (conn->synchronous) 
					{
						length = syncReadFromEth(conn, conn->sessionSocket, optClampMSS);
					} 
					else 
					{
						length = asyncReadFromEth(conn, conn->sessionSocket, optClampMSS);
					}
					if(length != -1)
					{
						uploadByte = uploadByte+(long)length/8-packetLenth;
						//syslog(LOG_ERR, "uploadByte:%d",length);
					}
				} 
				while (BPF_BUFFER_HAS_DATA);
				
				//syslog(LOG_ERR, "hezuoxiang:sessionSocket is in");
			}
		}
...
//get the time used
long timeCompare(long now,long last)
{
	long timeOfDate = 86400000;	//24*3600*1000
	if(now == last)
	{
		return 0;
	}
...
int testlog(char*log)
{
	char * filename = "/var/log/pppoe.log";
...
{
	char log[100] = "hezuoxiang:main is in\n";
	testlog(log);
	char addr[40];
    int opt;
    int n;
...
#ifdef DEBUGGING_ENABLED
    options = "I:VAT:D:hS:C:Usm:np:e:kdf:F:t:c:";
#else
    options = "I:VAT:hS:C:Usm:np:e:kdf:F:t:c:";
#endif
...
* device.
***********************************************************************/
int asyncReadFromEth(PPPoEConnection *conn, int sock, int clampMss)
{
    PPPoEPacket packet;
    int len;
    int plen;
...
* device.
***********************************************************************/
int syncReadFromEth(PPPoEConnection *conn, int sock, int clampMss)
{
    PPPoEPacket packet;
    int len;
    int plen;
