...
static void startPPPD(ClientSession *sess,int semid);
...
processPADR(Interface *ethif, PPPoEPacket *packet, int len)
{
	...
	//wait while addr is in shm
	Sem_Wait(semid);
	syslog(LOG_ERR, "After Sem_Wait\n");
	DestroySem(semid);
	return;
    }
...
int createDir(char * dir)
{
	...
	{
		//create dir
		printf("Create directory for shared memory use\n");
...
***********************************************************************/
void
startPPPDUserMode(ClientSession *session,int semid)
{
    /* Leave some room */
    char *argv[32];
    ...
    argv[c++] = "pppd";
    char mac[20];
    sprintf(mac,"%02x:%02x:%02x:%02x:%02x:%02x",session->eth[0], session->eth[1], session->eth[2],session->eth[3], session->eth[4], session->eth[5]);
    argv[c++] = mac;
    argv[c++] = "pty";

    char addr[20];
		memset(addr,0,20);
		sprintf(addr,"%d.%d.%d.%d",(int) session->peerip[0], (int) session->peerip[1],(int) session->peerip[2], (int) session->peerip[3]);

		//save addr in shm and post the signal
		speedLimit object;
		memset(&object,0,sizeof(speedLimit));
		strcpy(object.addr,mac);
		object.pid = -1;
		object.uploadSpeed = DEFAULT_SPEED_LIMIT_UPLOAD;
		object.downloadSpeed = DEFAULT_SPEED_LIMIT_DOWNLOAD;
		addr_set(object);
		
		syslog(LOG_ERR, "Before Sem_Post\n");
		Sem_Post(semid);
		DestroySem(semid);

    /* Let's hope service-name does not have ' in it... */
    snprintf(buffer, SMALLBUF, "%s -n -I %s -e %u:%02x:%02x:%02x:%02x:%02x:%02x%s -S '%s' -c %s",
	     PPPOE_PATH, session->ethif->name,
	     (unsigned int) ntohs(session->sess),
	     session->eth[0], session->eth[1], session->eth[2],
	     session->eth[3], session->eth[4], session->eth[5],
	     PppoeOptions, session->serviceName,addr);
...
***********************************************************************/
void
startPPPD(ClientSession *session,int semid)
{
    if (UseLinuxKernelModePPPoE) startPPPDLinuxKernelMode(session);
    else startPPPDUserMode(session,semid);
}
