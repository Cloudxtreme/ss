...
define("QUENE_NAME","pppoe_mgr_user_sim");
...
while(1)
{
	...
	else
	{
		...
		if(!$result)
		{
			$user->updateManage(User::COMMAND_SIM,false);
			continue;
		}
		$result = $user->handleCommand(User::COMMAND_SIM);
		if($result)
		{
			$user->updateManage(User::COMMAND_SIM,true);
		}
		else
		{
			$user->updateManage(User::COMMAND_SIM,false);
