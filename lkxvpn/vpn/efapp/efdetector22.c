...
static int ip_work_process(void *arg)
{
	...
	if(1)
	{
        ...
        mask = mask << ((1 + db->id * READER_WORKER_NUM * 2 + worker->id)/1);
    ...
    while(g_run)
    {
        if(worker->finish < worker->total)
        {
            unsigned char *pkg;
            unsigned int len;
            unsigned long finish, total, tasks = 0;
            ...
            while(finish < total)
            {
                pkg = worker->pkg_buf[worker->j];
                len = worker->pkg_len[worker->j];
...
static int session_work_process(void *arg)
{
	...
	{
        unsigned long mask = 1;
        mask = mask << ((1 + db->id * READER_WORKER_NUM * 2 + READER_WORKER_NUM + worker->id)/1);
		sched_setaffinity(0, sizeof(mask), &mask);
	}
    fprintf(stderr, "reader %d 's worker %d begin!\n", worker->reader->id, worker->id);
    ...
    while(g_run)
    {
        if(worker->finish < worker->total)
        {
            ...
            while(finish < total)
            {
            	pkg = worker->pkg_buf[worker->j];
                len = worker->pkg_len[worker->j];
                pkg_process(db, reader, pool, pkg, len);
                worker->j = (worker->j + 1 == READER_MAX_SLOT) ? 0 : (worker->j + 1);
                finish++;
                tasks++;
            }
...
static reader_t *hash_reader[0x100] = {0};
static int read_handle(int fd, ef_slot *slot, int num)
{
    int i;
    reader_t *reader = hash_reader[fd];
    lock(&(reader->lock));
    reader->pkg += num;
    ...
    for(i = 0; i < num; i++)
    {
        void *pkg = slot[i].pbuf;
        unsigned int len = slot[i].plen;
        ...
        if(IF_IP(pkg))
        {
            ...
            key1 = (reader->flag == READER_FLAG_INBOUND) ? (dip & (READER_WORKER_NUM - 1)) : (sip & (READER_WORKER_NUM - 1));
    ...
    for(i = 0; i < READER_WORKER_NUM; i++)
    {
        ///fprintf(stderr, "get:%u get:%u\n", reader->ip_worker[i]->get, reader->session_worker[i]->get);
...
static int read_pollback()
{
    int i = 0;
    ///fprintf(stderr, "poll back in-------------------------------------------!\n");
    while(1 && g_run)
    {
        if(g_reader[i]->finish == g_reader[i]->total)
        {
            if(++i == g_reader_num)
                break;
        }
...
static int read()
{
	...
	{
        reader_t *reader = g_reader[i];
        hash_reader[reader->fd] = reader;
        for(j = 0; j < READER_WORKER_NUM; j++)
        {
            reader->ip_worker[j] = init_worker(reader, j, WORKER_FLAG_IP);
            reader->session_worker[j] = init_worker(reader, j, WORKER_FLAG_SESSION);
            pthread_create(&(reader->ip_worker[j]->thread), NULL, ip_work_process, (void *)(reader->ip_worker[j]));
            pthread_create(&(reader->session_worker[j]->thread), NULL, session_work_process, (void *)(reader->session_worker[j]));
        }
	}
	efio_mbdg_start(read_handle, read_pollback, g_reader_num, g_reader[0]->fd, g_reader[1]->fd, g_reader[2]->fd, g_reader[3]->fd);
	for(i = 0; i < g_reader_num; i++)
	{
        reader_t *reader = g_reader[i];
        for(j = 0; j < READER_WORKER_NUM; j++)
        {
            pthread_join(reader->ip_worker[j]->thread, NULL);
            pthread_join(reader->session_worker[j]->thread, NULL);
            free_worker(reader->ip_worker[j]);
            free_worker(reader->session_worker[j]);
        }
	}
}
...
static int db_collecter(void *arg)
{
	...
	{
        unsigned long mask = 1;
		mask = mask << ((1 + g_database_num * 4 + db->id)/1);
		sched_setaffinity(0, sizeof(mask), &mask);
	}
	while(g_run)
	{
