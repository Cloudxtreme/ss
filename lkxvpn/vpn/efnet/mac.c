...
int mac_2_str(unsigned char *mac, unsigned char *str)
{
	int i = 6;
	int len;
	unsigned char *macch = (unsigned char *)mac;
	unsigned char v;
	...
	while(i--)
	{
		v = *macch++;
		//len = (v < 0x10) ? 1 : 2;
		//memcpy(str, mac_str[v], len);
		//(len == 2) ? (*(unsigned short*)str = *(unsigned short*)(&mac_str[v][0]))
		//		: (*str = mac_str[v][0]);
		len = 2;
		*(unsigned short*)str = *(unsigned short*)(&mac_str[v][0]);
		str += len;
		if(i)
			*str++ = ':';
	}
